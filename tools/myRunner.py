...
"""
一个自定义的指标类，可能封装了性能评估相关的功能（如记录准确率、比较当前指标与最佳指标）。
"""
class Acc_Metric:
...
def calculate_total_param(base_model):
    ...
    """
    遍历张量的形状（i.size()），通过将各个维度的大小相乘，计算参数张量的总元素数量。
    例如：
    如果张量形状为 (256, 512)，则总元素数为 256 × 512 = 131072。
    如果形状为 (10,)（向量），则总元素数为 10。
    """
    k = 0
    for i in params:
        l = 1
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    builder.dataset_builder：
        构建数据集、数据加载器以及采样器（分布式时需要 train_sampler）。
        train_dataloader 和 test_dataloader：
        分别用于训练和验证数据加载。
    """
